...
const Rolling = () => {
    ...
    const SLIDER_ITEM_GAP = 48; // 아이템 간격
    ...
    const slideRolling = (length: number, number: number) => {
        ...
        const random =
            Math.floor(Math.max(Math.random() * (MAX_WHEEL_COUNT - 1), 0)) +
            MIN_WHEEL_COUNT;
        ...
        for (let i = 1; i < random; i++) {
            rolling.push(
                {
                    offset: (1 / random) * i,
                    transform: `translate3d(calc(-${100 * length}% - ${
                        SLIDER_ITEM_GAP * length
                    }px), 0, 0)`,
                },
        ...
        return [
            ...
            {
                offset: 1,
                transform: `translate3d(calc(-${100 * number}% - ${
                    SLIDER_ITEM_GAP * number
                }px), 0, 0)`,
            },
        ];
    };
    ...
    useEffect(() => {
        startRollingAnimation(
            {
                duration: 3000,
                easing: "cubic-bezier(.63,0,.41,1.16)",
                fill: "forwards",
            },
    ...
    return (
        <>
            <div
                ...
                ref={sliderRef}
                style={
                    { "--gap": `${SLIDER_ITEM_GAP}px` } as React.CSSProperties
                }
            >
                <div className={S.slider__container} ref={sliderContainerRef}>
                    <div className={S.slider__list}>
                        {extendedSelectArray.map((value, index) => {
                            return (
                                <div
                                    className={cns(S.slider__item, "tossface")}
                                    key={index}
                                >
                                    {value.symbol}
                                </div>
            ...
            <div className={S.winning} ref={winningRef}>
                <div
                    className={cns(S.winning__item, "tossface")}
                    ref={winningIconRef}
                >
