...
const Select = () => {
    ...
    const renderList = () => {
        ...
        for (let i = 0; i < 3; i++) {
            result.push(
                <li className={S.select_menu_list__item} key={i}>
                    {reversedData[i] ? reversedData[i].symbol : <span>?</span>}
                    {i === 0 && selectArray.length > 3 && (
                        <span className={S.select_menu_list__overlay}>
        ...
        return <ul className={S.select_menu_list}>{result.reverse()}</ul>;
    };
    ...
    const handleSubmit = () => {
        if (selectArray.length >= 3) {
            navigate("/rolling", {
                state: {
                    select: selectArray,
                    winning: Math.floor(Math.random() * selectArray.length),
                },
            });
        }
    };
        ...
        <>
            <header className={S.header}>
                ...
                <p className={S.sub_title}>
                    지금 가장 먹고 싶은 메뉴들을 선택해주세요. <br />그 중
                    하나를 추천해드릴게요.
                </p>
            </header>
            <main className={S.body}>
                ...
                <div className={S.message}>
                    선택한 메뉴는 앞으로 더욱 맞춤화된 서비스를 위해 활용됩니다.
                </div>
            </main>
